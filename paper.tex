...
\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{DataOutput/analysis/loss_curves_anchors.png}
\caption{Validation-loss curves for the anchor configurations (all-off AdamW
baseline and all-on full Muon recipe, bold) and the leave-one-out runs (full
recipe with one toggle removed). The full-recipe band separates cleanly from the
baseline early and holds the gap for the rest of training; the highest
leave-one-out trajectory is the one dropping QK-norm.}
\label{fig:curves}
\end{figure}
