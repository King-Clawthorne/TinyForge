...
than the idea, and few changes helped across tasks. We share their motivation,
...
tasks at fixed scale to ask which modifications generalize. Ours fixes a single
backbone, corpus, and token budget and instead resolves the \emph{interactions}
within one contemporary stack, isolating that the bulk of its advantage lives in
a single Muon $\times$ QK-norm pair rather than spreading across the seven
toggles. The components we test are all recent and postdate that survey, namely
...
Gemma~2~\cite{gemma2}. Methodologically we adopt a fractional factorial design with matched
...
Our compute-constrained, single-GPU regime follows a line of work on small-scale
...
schedule. That regime is what lets us afford 66 runs, but it also bounds our
claims: it is precisely the short-schedule, small-vocab setting in which
stability components such as z-loss are expected to be inert, a caveat we return
